...
class DFE(nn.Module): #BiLSTM
    ...
    def __init__(self,input_size,output_size = 16):
        ...
        self.conv1 = nn.Conv1d(input_size, 64, kernel_size=3, padding=1, bias=False)  # torch.Size([700, 14
        self.bn1 = nn.BatchNorm1d(64)#归一化 加速模型的训练，把参数进行规范化的处理，让参数计算的梯度不会太小
        self.conv2 = nn.Conv1d(64, 32, kernel_size=3, padding=1, bias=False)  # torch.Size([700, 14
        self.bn2 = nn.BatchNorm1d(32)
        self.conv3 = nn.Conv1d(32, output_size, kernel_size=3, padding=1,bias=False)  # torch.Size([700, 14
    ...
    def forward(self, x):
        ...
        # out = self.conv3(out)
        # out = self.bn3(out)
        # x = self.relu(out)          #700 32 30- 700 64 30
        return x

class MFSAN(nn.Module):

    def __init__(self, num_classes=1, inplanes=14, planes=128, output_size=16,avgpool_size = 2):
        super(MFSAN, self).__init__()
        self.sharedNet = CFE(inplanes, planes)

        self.sonnet1 = DFE(planes, output_size)
        self.sonnet2 = DFE(planes, output_size)
        self.sonnet3 = DFE(planes, output_size)


        self.rul_fc_son1 = nn.Linear(16*30, num_classes)
        self.rul_fc_son2 = nn.Linear(16*30, num_classes)
        self.rul_fc_son3 = nn.Linear(16*30, num_classes)

        self.avgpool = nn.AvgPool1d(avgpool_size, stride=1,padding=1)
    ...
    def forward(self, data_src, data_tgt = 0, label_src = 0, alpha = 0, mark = 1):
        ...
        if self.training == True:
            ...
            # data_tgt_son1 = self.avgpool(data_tgt_son1)  ### 700 1 99
            data_tgt_son1 = data_tgt_son1.reshape([data_tgt_son1.shape[0],(int(data_tgt_son1.shape[1])*int(data_tgt_son1.shape[2]))])
            # data_tgt_son1 = data_tgt_son1.view(data_tgt_son1.size(0), -1) ### 700 99
            pred_tgt_son1 = self.rul_fc_son1(data_tgt_son1)
            pred_tgt_son1 = pred_tgt_son1.squeeze(1)

            data_tgt_son2 = self.sonnet2(data_tgt)
            data_tgt_son2= data_tgt_son2.reshape([data_tgt_son2.shape[0], (int(data_tgt_son2.shape[1]) * int(data_tgt_son2.shape[2]))])
            # data_tgt_son1 = data_tgt_son1.view(data_tgt_son1.size(0), -1) ### 700 99
            pred_tgt_son2 = self.rul_fc_son2(data_tgt_son2)
            pred_tgt_son2 = pred_tgt_son2.squeeze(1)

            data_tgt_son3 = self.sonnet3(data_tgt)
            data_tgt_son3 = data_tgt_son3.reshape([data_tgt_son3.shape[0],(int(data_tgt_son3.shape[1])*int(data_tgt_son3.shape[2]))])
            # data_tgt_son1 = data_tgt_son1.view(data_tgt_son1.size(0), -1) ### 700 99
            pred_tgt_son3 = self.rul_fc_son3(data_tgt_son3)
            pred_tgt_son3 = pred_tgt_son3.squeeze(1)

            if mark == 1:#源域数据1

                data_src = self.sonnet1(data_src).squeeze(1)
                data_src = data_src.reshape([data_src.shape[0], (int(data_src.shape[1]) * int(data_src.shape[2]))])
                # data_tgt_son1 = data_tgt_son1.view(data_tgt_son1.size(0), -1) ### 700 99
                pred_src = self.rul_fc_son1(data_src)
                pred_src = pred_src.squeeze(1)
            ...
            if mark == 2:
                ...
                data_src = self.sonnet2(data_src).squeeze(1)
                data_src = data_src.reshape([data_src.shape[0], (int(data_src.shape[1]) * int(data_src.shape[2]))])
                # data_tgt_son1 = data_tgt_son1.view(data_tgt_son1.size(0), -1) ### 700 99
                pred_src = self.rul_fc_son2(data_src)
                pred_src = pred_src.squeeze(1)
            ...
            if mark == 3:
                ...
                data_src = self.sonnet3(data_src).squeeze(1)
                data_src = data_src.reshape([data_src.shape[0], (int(data_src.shape[1]) * int(data_src.shape[2]))])
                # data_tgt_son1 = data_tgt_son1.view(data_tgt_son1.size(0), -1) ### 700 99
                pred_src = self.rul_fc_son3(data_src)
                pred_src = pred_src.squeeze(1)
                # print(data_src.shape, data_tgt_son2.shape)
            ...
            fea_son1 = self.sonnet1(data).squeeze(1)
            fea_son1 = fea_son1.reshape([fea_son1.shape[0], (int(fea_son1.shape[1]) * int(fea_son1.shape[2]))])
            pred1 = self.rul_fc_son1(fea_son1)
            ...
            fea_son2 = self.sonnet2(data).squeeze(1)
            fea_son2 = fea_son2.reshape([fea_son2.shape[0], (int(fea_son2.shape[1]) * int(fea_son2.shape[2]))])
            pred2 = self.rul_fc_son2(fea_son2)
            pred2 = pred2.squeeze(1)

            fea_son3 = self.sonnet3(data).squeeze(1)
            fea_son3 = fea_son3.reshape([fea_son3.shape[0], (int(fea_son3.shape[1]) * int(fea_son3.shape[2]))])
            pred3 = self.rul_fc_son3(fea_son3)
