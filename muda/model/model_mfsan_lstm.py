...
from muda.core.options import Options
import random
import os
import numpy as np
opt = Options().parse()
...
class CFE(nn.Module): #一维卷积？？
    ...
    def __init__(self, inplanes, planes,hidden_size=64,dropout_prob=0.2,stride=1, downsample=None):
    ...
    def forward(self, x):
        ...
        # out = out[-1, -1, :]  #把700 64 30 变成 700 30 64   #难道不是700 50 128？
        # print(out.shape[-1])
        return out
...
class MFSAN(nn.Module):
    ...
    def __init__(self, num_classes=1, inplanes=14, planes=64, hidden_size=100,avgpool_size = 2):
