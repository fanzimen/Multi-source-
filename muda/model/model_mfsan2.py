    ...
    def forward(self, data_src, data_tgt = 0, label_src = 0, alpha = 0, mark = 1):
        ...
        if self.training == True:
            ...
            if mark == 1:#源域数据1
                ...
                mat = torch.cat((pred_tgt_son1.unsqueeze(1), pred_tgt_son2.unsqueeze(1)), 1)
                # print("mat.shape",mat.shape)
                mat_sca = F.normalize(mat, p=1, dim=1) #除以tensor的L1范数
                pred_tgt_son1_sca, pred_tgt_son2_sca = mat_sca[:, 0], mat_sca[:, 1]
                #源域1回归与另外两个回归器的误差
                ...
                return rul_loss, mmd_loss, l1_loss
