...
def writing_settings_2src(now, opt, model, f_mfsan_train=None):
    print("当前日期和时间：", now, file=f_mfsan_train, flush=True)
    print('training settings:\t', 'lr:', opt.learning_rate, 'momentum:', opt.momentum, 'l2_decay:',opt.l2_decay, 'optimizer:', opt.optimizer, 'seed:', opt.seed,
           file=f_mfsan_train, flush=True)
    print('model architecture:\n', model, file=f_mfsan_train, flush=True)
    print(opt.source_data_name1,opt.source_data_name2, "to", opt.target_data_name, file=f_mfsan_train, flush=True)
    #
...
def figure_generate_2src(current_dir,history,now,target_name,pred,target):
    # Generate the figure
    fig = plt.figure(figsize=(15, 12))
    plt.subplot(2, 2, 1)
    plt.xlabel('Epoch')
    ...
    print("ALL is finished!!!")
    # Save the plots to a file
    plt.savefig(current_dir+'/outputs/figures/train_loss/'+now+"_target_"+target_name+'_2src'+'.png')
