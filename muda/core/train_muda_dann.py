...
def train(model,max_len,source1_loader,source2_loader,source3_loader,target_train_loader):
    global dann_loss1,dann_loss2,dann_loss3 #全局
    print("--------------------------MFSAN --------------------------------")

    batch_src1,batch_src2,batch_src3,batch_tar = 0,0,0,0
    running_loss_scr1, running_loss_scr2 ,running_loss_scr3= 0, 0, 0
    running_dann_loss_scr1, running_dann_loss_scr2, running_dann_loss_scr3 = 0, 0, 0
    ...
    for batch in range(1, max_len):
        ...
        rul_loss1, dann_loss1, l1_loss1 = model(source_data1, target_data1, source_label1, alpha, mark=1)
        gamma = 2 / (1 + math.exp(-10 * (batch) / (max_len))) - 1

        loss1 = rul_loss1 + gamma * (dann_loss1 + l1_loss1)
        loss1.backward()
        optimizer.step()
        running_loss_scr1 += loss1.item() #loss为什么要加item() 直接输出的话数据类型是Variable,
        running_dann_loss_scr1 += dann_loss1.item()
        ...
        if batch % opt.log_interval == 0:
            print('Train source1 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tRUL_Loss: {:.6f}\tmmd_Loss: {:.6f}\tl1_Loss: {:.6f}'.format(
                batch, 100. * batch/ max_len, loss1.item(), rul_loss1.item(), dann_loss1.item(), l1_loss1.item()), file=f_mfsan_train, flush=True)
        ...
        rul_loss2, dann_loss2, l1_loss2 = model(source_data2, target_data2, source_label2, alpha, mark=2)
        gamma = 2 / (1 + math.exp(-10 * (batch) / (max_len))) - 1
        loss2 = rul_loss2 + gamma * (dann_loss2 + l1_loss2)
        loss2.backward()
        optimizer.step()
        running_loss_scr2 += loss2.item()
        running_dann_loss_scr2 += dann_loss2.item()
        ...
        if batch % opt.log_interval == 0:
            print(
                'Train source2 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tRUL_Loss: {:.6f}\tmmd_Loss: {:.6f}\tl1_Loss: {:.6f}'.format(
                    batch, 100. * batch / max_len, loss2.item(), rul_loss2.item(), dann_loss2.item(), l1_loss2.item()), file=f_mfsan_train, flush=True)
        ...
        rul_loss3, dann_loss3, l1_loss3 = model(source_data3, target_data3, source_label3, alpha, mark=3)
        gamma = 2 / (1 + math.exp(-10 * (batch) / (max_len))) - 1
        loss3 = rul_loss3 + gamma * (dann_loss3 + l1_loss3)
        loss3.backward()
        optimizer.step()
        running_loss_scr3 += loss3.item()
        running_dann_loss_scr3 += dann_loss3.item()
        ...
        if batch % opt.log_interval == 0:
            print(
                'Train source3 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tRUL_Loss: {:.6f}\tmmd_Loss: {:.6f}\tl1_Loss: {:.6f}\n\n'.format(
                    batch, 100. * batch / max_len, loss3.item(), rul_loss3.item(), dann_loss3.item(), l1_loss3.item()),
    ...
    epoch_dann_loss_scr1 = running_dann_loss_scr1 / (max_len - 1)
    epoch_dann_loss_scr2 = running_dann_loss_scr2 / (max_len - 1)
    epoch_dann_loss_scr3 = running_dann_loss_scr3 / (max_len - 1)
    ...
    return epoch_loss_scr1, epoch_loss_scr2, epoch_loss_scr3, epoch_dann_loss_scr1, epoch_dann_loss_scr2, epoch_dann_loss_scr3, epoch_l1_loss_scr1, epoch_l1_loss_scr2, epoch_l1_loss_scr3
...
if __name__ == '__main__':
    ...
    history['total_dann_loss'] = []
    history['total_l1_loss'] = []
    history['epoch_rul_loss_scr1'] = []
    history['epoch_rul_loss_scr2'] = []
    history['epoch_rul_loss_scr3'] = []
    history['epoch_dann_loss_scr1'] = []
    history['epoch_dann_loss_scr2'] = []
    history['epoch_dann_loss_scr3'] = []
    ...
    for epoch in range(1,opt.epochs+1):
        ...
        epoch_loss_scr1,epoch_loss_scr2,epoch_loss_scr3,epoch_dann_loss_scr1,epoch_dann_loss_scr2,epoch_dann_loss_scr3,epoch_l1_loss_scr1,epoch_l1_loss_scr2,epoch_l1_loss_scr3 = train(model,max_len,source1_loader,source2_loader,source3_loader,target_train_loader)
        ...
        history['total_dann_loss'].append((epoch_dann_loss_scr1+epoch_dann_loss_scr2+epoch_dann_loss_scr3) / 3)
        ...
        history['epoch_dann_loss_scr1'].append(epoch_dann_loss_scr1)
        history['epoch_dann_loss_scr2'].append(epoch_dann_loss_scr2)
        history['epoch_dann_loss_scr3'].append(epoch_dann_loss_scr3)
    ...
    figure_generate_dann(current_dir, history, now, opt.target_data_name, pred, target)