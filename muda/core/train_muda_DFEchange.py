...
import data_loader
from muda.model import model_mfsan_DFEchange as model_mfsan
...
from muda.utils.print_things import figure_generate,writing_settings
import pandas as pd
import os
import torch
...
if __name__ == '__main__':
    ...
    #数据写入
    f_mfsan_train = log_in_file('\\' +opt.target_data_name+ '_' + now + '_DFE_change_train.log')
    f_mfsan_test = log_in_file('\\' + opt.target_data_name+ '_' + now + '_DFE_change_test.log')
    ...
    print("DFE_change_",opt.source_data_name1, opt.source_data_name2, opt.source_data_name3, "to", opt.target_data_name,
    ...
    figure_generate(current_dir, history, now, opt.target_data_name, pred, target)